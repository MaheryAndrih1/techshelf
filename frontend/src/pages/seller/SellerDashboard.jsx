...
const SellerDashboard = () => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      // Try different endpoints to ensure we get data
      let ordersResponse;
      
      try {
        // First attempt with /seller-orders/
        console.log("Attempting to fetch orders with /seller-orders/");
        ordersResponse = await api.get('/seller-orders/');
      } catch (orderErr) {
        console.warn("Failed to fetch from /seller-orders/, trying alternative endpoint", orderErr);
        try {
          // Second attempt with /orders/seller/
          ordersResponse = await api.get('/orders/seller/');
        } catch (secondErr) {
          console.warn("Failed with /orders/seller/, trying final alternative", secondErr);
          // Final attempt
          ordersResponse = await api.get('/orders/seller-orders/');
        }
      }
      
      console.log("Orders data retrieved:", ordersResponse.data);
  ...
  const fetchSellerData = useCallback(async () => {
    try {
      ...
      // Fetch reports
      try {
        const reportsResponse = await api.get('/notifications/reports/');
        setReports(reportsResponse.data.results || reportsResponse.data);
      } catch (reportErr) {
        console.warn("Failed to fetch reports:", reportErr);
        setReports([]);
      }
  ...
  useEffect(() => {
    ...
    // Set up a refresh interval to check for new orders every 30 seconds
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-4">
              ...
              <Link to="/seller/reports" className="block">
                <CustomButton type="outline" fullWidth>
                  <svg className="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              <h3 className="text-md font-semibold text-[#33353a] mb-4">Recent Reports</h3>
